...
3. Write to `_temp/visual-diff.html` unless the user supplies an output path. `_temp/` is already listed in this project's `.gitignore`, so generated pages stay out of version control.
4. Run the bundled generator from the repository root:
   ...
   Useful flags: `--no-unchanged` omits the collapsed panels for unchanged files; `--open` opens the page in a browser when done.
...
If the user asks for a commit message for the reviewed changes, follow this project's `AGENTS.md` convention: an imperative subject plus a body explaining what changed and why. Do not commit or push unless explicitly asked.

## Resource

`scripts/generate-split-view-diff.py` is the bundled generator; it needs only the Python standard library and Git on `PATH`. The page is self-contained apart from the optional highlight.js syntax-highlighting script it loads from a CDN — without network access the page still renders, just without colored tokens.
